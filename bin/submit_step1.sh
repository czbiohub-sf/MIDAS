...
#$ -t 1-1000                        #-- Array job: submit XX jobs to the job queues at once.
#$ -tc 1000                         #-- specify the maximum number of concurrent tasks
...
${GNUTIME} -v python -m midas2 annotate_genome --genomes ${SGE_TASK_ID}:${GENOME_STEP} \
  ...
  --scratch_dir ${SCRATCH_DIR} --num_threads ${TREADS} --debug --force &> ${OUTDIR}/annotate_time.log
