...
#$ -t 1-500                        #-- Array job: submit XX jobs to the job queues at once.
#$ -tc 500                         #-- specify the maximum number of concurrent tasks


module load Sali anaconda
conda activate midas2v1.05
...
do
  ...
  midas2 annotate_genome --genomes ${genome_id} \
    --midasdb_name newdb --midasdb_dir ${MIDASDB_DIR} \
    --scratch_dir ${GENOME_DIR} --num_threads ${TREADS} --debug
